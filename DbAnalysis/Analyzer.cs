	...
	public class Analyzer
	{
		...
		public Analyzer (DatabaseContext DatabaseContext)
		{
			...
			var PSignPrefix =
					...
					where !res.Contains ("--")
					select res.SourcedTextSpan (c, sp);
			...
			var PBaseAtomicST =
					PNull.SqlToken ().ProduceType (DatabaseContext.TypeMap.Null)
						...
						.Or (PQualifiedIdentifierLST
							.Select<Sourced<string>[], Func<RequestContext, NamedTyped>> (p => rc =>
							{
			...
			var PAtomicST =
					...
					.Or (
						from f in SqlToken ("json_object_agg")
						from args in PExpressionRefST.Get.CommaDelimitedST ().InParentsST ()
						where args.Count () == 2
						select (Func<RequestContext, NamedTyped>)(rc =>
							new NamedTyped (f, DatabaseContext.TypeMap.Json.SourcedCalculated (f)))
					...
					.Or (
						from f in SqlToken ("jsonb_object_agg")
						from args in PExpressionRefST.Get.CommaDelimitedST ().InParentsST ()
						where args.Count () == 2
						select (Func<RequestContext, NamedTyped>)(rc =>
							new NamedTyped (f, DatabaseContext.TypeMap.Jsonb.SourcedCalculated (f)))
			...
			var PValuesSourceST =
					...
					select new ValuesBlock (v.Value.ToArray (),
						table_name,
						column_names.GetOrEmpty ())
				;
